<template>
      ...
      <v-row>
        <v-col>
          ...
          >
            ...
            <template v-if="resource === 'Clients'" v-slot:[`item.logo`]="{ item }">
              <v-img
                :src="item.logo"
                aspect-ratio="1"
                width="50"
  ...
  export default class AppTable extends Vue {
    ...
    async fetchData() {
      ...
      return {
        ...entity,
        createdAt: entity.createdAt ? new Date(entity.createdAt).toISOString().split('T')[0] : null,
        updatedAt: entity.updatedAt ? new Date(entity.updatedAt).toISOString().split('T')[0] : null,
      };
    });
